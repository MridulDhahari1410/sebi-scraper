...
func (r *redisClusterCacheClient) IncrementAll(ctx context.Context, _ time.Duration, keys ...string) (result map[string]int64, err error) {
	ts := time.Now()
	defer r.log(ctx, ts, err)
	// create the result
	...
	// fetch the result
	for _, key := range keys {
		_, err := pipeline.Incr(ctx, key).Result()
		if err != nil {
			return nil, err
		}
	}
